...
%BLENDIMAGEPAIR
    ...
    binary_mask_blend_s = masks == 0;
    binary_mask_blend_d = maskd == 0;
    ...
    for c = 1:Cs
        ...
        elseif strcmp(mode, 'blend')
            ...
            dist_s = bwdist(binary_mask_blend_s,'euclidean');
            
            % normalize
            temp = dist_s - min(dist_s(:));
            norm_dist_s = temp ./ max(temp(:));
            
            dist_d = bwdist(binary_mask_blend_d,'euclidean');
            
            % normalize
            temp = dist_d - min(dist_d(:));
            norm_dist_d = temp ./ max(temp(:));
            
            channel_out = (S .* norm_dist_s + D .* norm_dist_d) ./ (norm_dist_s + norm_dist_d);
        end
