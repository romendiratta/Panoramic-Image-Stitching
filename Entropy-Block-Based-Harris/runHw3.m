...
signAcademicHonestyPolicy('Andrew Degler', 'adegler');
...
% -------------------
portrait_pts = [6 8; 322 6; 8 395; 323 394];
bg_pts = [97 20; 273 76; 85 441; 279 421];
close all;
%[prt_x, prt_y] = getPointsFromUser(portrait_img, 4, 'Click any 4 points');
%[bg_x, bg_y] = getPointsFromUser(bg_img, 4, 'Click the corresponding locations of the previous 4 points');

%disp([bg_x, bg_y]);

%portrait_pts = [prt_x, prt_y];
%bg_pts = [bg_x, bg_y];
...
%imgc = imadjust(imgc,[0 1],[0 0.05]);
%imgl = imadjust(imgl,[0 1],[0 0.05]);
%imgr = imadjust(imgr,[0 1],[0 0.05]);
...
%figure, imshow(stitched_img);
imwrite(stitched_img, 'street_panorama.png');