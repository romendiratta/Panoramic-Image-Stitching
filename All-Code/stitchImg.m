...
%               get you started.
    ...
    for n = 1:num_imgs
        ...
        %disp("here");
        imwrite(img_n, 'key_city_left.png');
        imwrite(stitched_img, 'key_city_center.png');
        
        %stitched_img = imread('key_city_center3.png');
        %img_n = imread('key_city_left3.png');

        [kp_stitched, kp_n] = genSIFTMatches(stitched_img, img_n);
        %EBBMatches = getEBBHarrisMatches(rgb2gray(stitched_img), rgb2gray(img_n));
        %SURFMatches = getSURFMatches(rgb2gray(stitched_img), rgb2gray(img_n));
        %HarrisMatches = getHarrisMatches(rgb2gray(stitched_img), rgb2gray(img_n));
        %KeyMatches = getKeyMatches();

        %stitched_combined = [kp_stitched ; EBBMatches.stitched];
        %n_combined = [kp_n ; EBBMatches.n];
       
        
        % get the homography matrix
        %[inliers_id, H_3x3] = runRANSAC(KeyMatches.n, KeyMatches.stitched, 15000, 1);
        [inliers_id, H_3x3] = runRANSAC(kp_n, kp_stitched, 15000, 1);
        %[inliers_id, H_3x3] = runRANSAC(EBBMatches.n, EBBMatches.stitched, 15000, 1);
        %[inliers_id, H_3x3] = runRANSAC(SURFMatches.n, SURFMatches.stitched, 15000, 1);
        %[inliers_id, H_3x3] = runRANSAC(HarrisMatches.n, HarrisMatches.stitched, 15000, 1);
        %[inliers_id, H_3x3] = runRANSAC(n_combined, stitched_combined, 100, 3);
        
        % warp the image 
        dest_canvas_width_height = [size(stitched_img, 2), size(stitched_img, 1)];
        [dest_mask, dest_img] = backwardWarpImg(img_n, inv(H_3x3), dest_canvas_width_height);
        
        stitch_mask = rgb2gray(stitched_img) > 0;
        ...
        % blend the image with the stitched image
        stitched_img = blendImagePair(stitched_img, stitch_mask, dest_img, dest_mask, ...
        'blend');
    end
end